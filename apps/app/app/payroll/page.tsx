  ...
  return (
    <main style={themeVars} className="relative min-h-svh">
      ...
      <div className="relative max-w-[1600px] mx-auto px-6 md:px-12 py-16">
        ...
        <form method="post" action="/api/payroll/run" className="flex flex-wrap items-end gap-3 border border-line bg-surface p-6 mb-10">
          <label className="flex flex-col gap-1 font-body text-xs text-mute-2 uppercase tracking-widest">
            Period
            <input name="period" type="month" required className="border border-line bg-ink px-4 py-3 font-body text-sm text-chalk focus:outline-none focus:border-brand" />
          </label>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-line border border-line mb-10">
              {[
                ...
                <div key={s.v} className="bg-ink px-6 py-5 hover:bg-brand-50 transition-colors">
                  <div className="font-display text-2xl md:text-3xl text-brand">{s.k}</div>
                ...
                <thead>
                  <tr className="border-b border-line bg-surface text-left">
                    {['Employee', 'Gross', 'No-pay d', 'No-pay ded.', 'EPF 8%', 'EPF 12%', 'ETF 3%', 'APIT', 'Net'].map((h) => (
                      <th key={h} className="px-4 py-3 font-body text-xs uppercase tracking-widest text-mute-2">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
