...
const input =
  'border border-line bg-ink px-3 py-2 font-body text-sm text-chalk placeholder:text-mute-3 focus:outline-none focus:border-brand';
  ...
  return (
    <main className="relative min-h-svh">
      ...
      <div className="relative max-w-[1600px] mx-auto px-6 md:px-12 py-16">
        ...
        <h1 className="font-display text-chalk leading-[0.92] mt-4 mb-10" style={{ fontSize: 'clamp(40px, 5vw, 72px)' }}>
          PROSPECTS
        </h1>
        ...
        <div className="space-y-4">
          ...
          {prospects.map((p) => (
            <div key={p.id} className="border border-line bg-ink p-6 flex flex-wrap items-center gap-6 hover:bg-brand-50 transition-colors">
              <div className="min-w-64">
                <div className="font-display text-2xl text-chalk tracking-wide">{p.company_name.toUpperCase()}</div>
              ...
              {p.converted_tenant_id ? (
                ...
                <form method="post" action={`/api/prospects/${p.id}/convert`} className="ml-auto flex flex-wrap items-end gap-3">
                  ...
                  <button
                    type="submit"
                    className="px-6 py-2.5 bg-brand-gradient text-white font-display text-sm tracking-widest uppercase shadow-brand"
                  >
                    Convert to tenant
                  </button>
                </form>
